...
class Database:    
    ...
    # --- MEMBER BOOSTER DB METHODS ---
    async def get_booster_count(self, chat_id, user_id):
        # Create a new collection dynamically for booster stats
        booster_col = self.db['booster_stats']
        doc = await booster_col.find_one({"chat_id": chat_id, "user_id": user_id})
